...
const HASHED_PASSWORD_KEY = "f4388bd5333b63f6b268e7dae9300b95617b40fe7027b4e6df7cdbc75c1cac26"; 
...
async function checkPassword() {
    ...
    while (attempts < MAX_ATTEMPTS) {
        password = prompt("Masukkan Sandi untuk Akses Player Skylord Vibrasi:");
        
        if (password === null) {
            OVERLAY_ELEMENT.innerHTML = "<h2>AKSES DITOLAK</h2><p>Refresh halaman untuk mencoba lagi.</p>";
            return;
        }

        OVERLAY_ELEMENT.innerHTML = "Memverifikasi Sandi...";
        
        const inputHash = await sha256(password); 

        if (inputHash === HASHED_PASSWORD_KEY) {
            ...
            attempts++;
            alert(`Sandi Salah! Anda memiliki ${MAX_ATTEMPTS - attempts} percobaan tersisa.`);
        }
    }
    
    OVERLAY_ELEMENT.innerHTML = "<h2>AKSES DITOLAK</h2><p>Percobaan sandi habis. Refresh halaman untuk mencoba lagi.</p>";
}
...
-SE
findcenter:      pink/50 100+9.45/50
consciousnessplus:    pink/40 150+4.5/60
limitless:    pink/30 175+7.1/70
deviodtune:    pink/20 200+22/80
headplus:    pink/10 225+9.5/90
outerview:    pink/5 250+3.2/95
alloff:       -

NOW findcenter
+00:03:00 findcenter ->
+00:08:00 consciousnessplus
+00:10:00 consciousnessplus ->
+00:20:00 limitless
+00:22:00 limitless ->
+00:28:00 deviodtune
+00:30:00 deviodtune ->
...
function loadExternalAudio(file, context) {
    if (!file) {
        externalAudioBuffer = null;
        return Promise.resolve();
    }
    
    statusDiv.textContent = `Memuat audio eksternal: ${file.name}...`;
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (e) => {
            context.decodeAudioData(e.target.result, 
                (buffer) => {
                    ...
                    statusDiv.textContent = `Audio eksternal ${file.name} berhasil dimuat.`;
                    resolve();
                },
                (error) => {
                    externalAudioBuffer = null;
                    statusDiv.textContent = `Gagal decode audio eksternal: ${error.message}`;
                    reject(error);
                }
            );
        };
        reader.onerror = (error) => {
            externalAudioBuffer = null;
            statusDiv.textContent = 'Gagal membaca file WAV.';
            reject(error);
        };
...
function updateStatusAndTimer() {
    ...
    timerDiv.textContent = `⏳ ${formattedTime} / ${formatDuration(totalDuration)}`;

    if (elapsedTime >= totalDuration) {
        if (isLooping && sbgData) { 
            stopAudio();
            startAudio();
        } else {
            stopAudio(); 
    ...
    if (sbgData && sbgData.events.length) {
        ...
        if (preset) {
            statusDiv.textContent = `Menjalankan Frekuensi: ${preset.freqL.toFixed(2)} Hz / ${preset.freqR.toFixed(2)} Hz`;
        }
    } else if (externalAudioBuffer) {
         statusDiv.textContent = `Memutar Audio Eksternal...`;
    }
}
    ...
    audioContext.resume().then(() => {
        ...
        if (externalAudioBuffer) {
            ...
            if (isExternalLoopingToFillTime) {
                ...
                externalAudioSource.onended = function() {
                    if (isPlaying) { 
                        const newSource = audioContext.createBufferSource();
                        newSource.buffer = externalAudioBuffer;
                        newSource.connect(externalGainNode);
                        newSource.onended = this.onended;
                        newSource.start(0);
                        externalAudioSource = newSource;
                    }
                };
            } else {
                externalAudioSource.start(0, startOffset); 
...
function stopAudio() {
    ...
    if (externalAudioSource) {
        try {
            externalAudioSource.onended = null; 
    ...
    timerDiv.textContent = `⏳ 00:00 / ${formatDuration(totalDuration)}`;
    statusDiv.textContent = 'Selamat Program Telah Selesai !!!';
...
function initializeInternalSbg() {
    ...
    try {
        ...
        if (sbgData && sbgData.events.length > 0) {
            ...
            timerDiv.textContent = ` ⏳ 00:00 / ${formatDuration(totalDuration)}`;
            statusDiv.textContent = `Program '${sbgNameInput.value}' Siap. Klik Play Sekarang`;
            ...
            if (externalAudioBuffer) {
                ...
                timerDiv.textContent = ` ⏳ 00:00 / ${formatDuration(totalDuration)}`;
                statusDiv.textContent = `Hanya Audio Eksternal Siap. Klik Play Sekarang`;
                playBtn.disabled = false;
            } else {
                statusDiv.textContent = 'Gagal memproses data internal. Periksa format kode.';
                playBtn.disabled = true;
            }
        }
    } catch (error) {
...
externalWavInput.addEventListener('change', (event) => {
    ...
    loadExternalAudio(file, audioContext).then(() => {
        initializeInternalSbg(); 
    }).catch(error => {
        console.error(error);
    });
...
async function downloadPinkNoise() {
    ...
    try {
        for (let i = 0; i < totalSegments; i++) {
            ...
            statusDiv.textContent = `Rendering Pink Noise segmen ${i + 1} dari ${totalSegments} (${((i + 1) / totalSegments * 100).toFixed(0)}%)...`;
...
async function downloadAudio(channel) {
    ...
    try {
        for (let i = 0; i < totalSegments; i++) {
            ...
            statusDiv.textContent = `Rendering segmen ${i + 1} dari ${totalSegments} (${((i + 1) / totalSegments * 100).toFixed(0)}%)...`;
